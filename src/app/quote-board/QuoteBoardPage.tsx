...
export default function QuoteBoardPage() {
  // Tea wall: room is private
  useRequireAuth("/quote");
  ...
  async function postReply(messageId: string) {
    ...
    if (!authData?.user) {
      setReplyErrorByMessage((p) => ({ ...p, [messageId]: "Login required." }));
      return;
    }
    ...
    <div className="min-h-screen bg-gradient-to-b from-zinc-50 to-white text-zinc-900 dark:from-black dark:to-zinc-950 dark:text-zinc-50">
      ...
      <main className="mx-auto max-w-4xl px-6 pb-20 pt-12">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">Room</h1>
            <p className="mt-2 text-zinc-600 dark:text-zinc-300">
              Shared chat room — newest on top{loading ? " — loading…" : ""}
            </p>
          </div>
        ...
        <section className="mt-6 grid gap-4 sm:grid-cols-2">
          <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
            ...
            <p className="mt-2 text-3xl font-semibold">{mode === "supabase" ? messages.length : 0}</p>
            <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">messages loaded</p>
          </div>
          <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
            <p className="text-sm font-medium text-zinc-600 dark:text-zinc-300">Leaderboard</p>
        ...
        <section className="mt-8 grid gap-6">
          {latest3.map((q, i) => (
            ...
            >
              <p className="text-sm font-semibold text-zinc-500 dark:text-zinc-400">Latest #{i + 1}</p>
          ...
          <div className="mt-4 grid gap-4">
            {mode === "supabase"
              ? messages.map((m) => {
                  ...
                  const replyFriend =
                    (replyFriendByMessage[m.id] as FriendName | undefined) ?? friends[0];
                    ...
                    >
                      ...
                      <div className="mt-4 rounded-2xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-black/40">
                        ...
                        {replies.length ? (
                            ...
                            {replies.map((r) => (
                              ...
                              >
                                <div className="flex items-center justify-between gap-2">
                                  <span className="text-xs font-semibold">{r.friend}</span>
                          ...
                          <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">No replies yet.</p>
                        )}

                        <div className="mt-4 grid gap-3">
                          <div className="grid gap-3 sm:grid-cols-2">
                            <label className="grid gap-1">
                              <span className="text-xs font-medium text-zinc-600 dark:text-zinc-300">Friend</span>
                              <select
                                value={replyFriend}
                                onChange={(e) =>
                            ...
                            <label className="grid gap-1">
                              <span className="text-xs font-medium text-zinc-600 dark:text-zinc-300">4-digit code</span>
                              <input
                                value={replyPin}
                                onChange={(e) => {
                          ...
                          <label className="grid gap-1">
                            <span className="text-xs font-medium text-zinc-600 dark:text-zinc-300">Reply</span>
                            <textarea
                              value={replyText}
                              onChange={(e) =>
                          ...
                          {rerr ? (
                            <p className="text-xs text-red-600 dark:text-red-400">{rerr}</p>
                          ) : null}
